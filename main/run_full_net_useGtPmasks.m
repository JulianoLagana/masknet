function [ instances ] = run_full_net_useGtPmasks( imgs, imgIds, clssegs, varargin )
...
%   Detailed explanation goes here
    ...
    % Use ground truth data as the semantic segmentation
    if opts.verbose
        fprintf(' (%.3fs)',toc);
        tic;
        fprintf('\nloading gt data for semantic segmentation...');
    end    
    segFCN = clssegs;
    ...
    for i = 1 : numel(imgs)
        ...
        for j = 1 : size(detections{i},1)
            ...
            pMask = generatePartialMask(detections{i}, j, segFCN{i}+1, masknetInputSize); 
            ...
            % Display debug info
            if opts.debug
                subplot(1,3,1); imshow(patch);
                subplot(1,3,2); imagesc(pMask);
                subplot(1,3,3); imagesc(mask);
                waitforbuttonpress;
            end
