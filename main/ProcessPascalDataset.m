    ...
    while ~isempty(allIds)
        ...
        for i = 1 : numel(ids)
            ...
            for j = 1 : size(boxes{i},1)
                ...
                % If it's a failed bbox, skip it
                if sum(sum(gtMask==1)) == 0
                    continue;
                end
...
function plotDebugInfo(img, clsseg, segFCN, boxes, gtBoxes, patch, pMask, gtMask, j)
        ...
        for k = 1 : size(boxes,1)
            ...
            text(boxes(k,1), boxes(k,2)-5, [num2str(k) ' - ' cat_names{boxes(k,end)}], 'FontSize', 10, 'Color', 'r');
        end
